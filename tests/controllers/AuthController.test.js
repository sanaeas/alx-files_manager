...
describe('+ AuthController', () => {
  ...
  before(function (done) {
    ...
    dbClient.usersCollection()
      .then((usersCollection) => {
        usersCollection.deleteMany({ email: userExp.email })
          .then(() => {
            request.post('/users')
              .send({
                email: userExp.email,
                password: userExp.password,
              })
              .expect(201)
              .end((requestErr, res) => {
                if (requestErr) {
                ...
                expect(res.body.email).to.eql(userExp.email);
  ...
  describe('+ GET: /connect', () => {
    ...
    it('+ Fails with a valid email and wrong password', function (done) {
      this.timeout(5000);
      request.get('/connect')
        .auth(userExp.email, 'raboof', { type: 'basic' })
        .expect(401)
        .end((err, res) => {
          if (err) {
            return done(err);
    ...
    it('+ Fails with an invalid email and valid password', function (done) {
      this.timeout(5000);
      request.get('/connect')
        .auth('zoro@strawhat.com', userExp.password, { type: 'basic' })
        .expect(401)
        .end((err, res) => {
          if (err) {
            return done(err);
    ...
    it('+ Succeeds for an existing user', function (done) {
      this.timeout(5000);
      request.get('/connect')
        .auth(userExp.email, userExp.password, { type: 'basic' })
        .expect(200)
        .end((err, res) => {
          if (err) {
            return done(err);
